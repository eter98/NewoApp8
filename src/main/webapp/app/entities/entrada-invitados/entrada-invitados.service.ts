...
export class EntradaInvitadosService {
  ...
  protected convertDateFromClient(entradaInvitados: IEntradaInvitados): IEntradaInvitados {
    const copy: IEntradaInvitados = Object.assign({}, entradaInvitados, {
      registroFecha:
        entradaInvitados.registroFecha != null && entradaInvitados.registroFecha.isValid() ? entradaInvitados.registroFecha.toJSON() : null
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((entradaInvitados: IEntradaInvitados) => {
        entradaInvitados.registroFecha = entradaInvitados.registroFecha != null ? moment(entradaInvitados.registroFecha) : null;
      });
    }
    return res;
  }
